...
class pid_velocity_fixed_height_controller():
    ...
    def pid(
        ...
        desired_pitch = (gains["kp_vel_xy"] * np.clip(vx_error, -1, 1) + 
                gains["kd_vel_xy"] * vx_deriv)
        desired_roll = (-gains["kp_vel_xy"] * np.clip(vy_error, -1, 1) - 
                gains["kd_vel_xy"] * vy_deriv)
        ...
        pitch_command = (-gains["kp_att_rp"] * np.clip(pitch_error, -1, 1) - 
                gains["kd_att_rp"] * pitch_deriv)

        yaw_command = (
                gains["kp_att_y"] * 
                np.clip(yaw_rate_error, -yawcap, yawcap))
